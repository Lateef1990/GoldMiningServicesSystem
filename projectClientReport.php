...
<?php
	...
	$stmt = $db_con->prepare('SELECT e.Client_id, e.FullName, u.Location, u.Client_id, u.DateCommenced,u.DateCompleted FROM `client` AS e INNER JOIN `project` AS u ON e.Client_id = u.Client_id AND e.Client_id = :uClient_id');
    $stmt->execute(array(":uClient_id"=>$_SESSION['Client_id']));
    $stmt->execute();
