...
<?php
	...
	$stmt = $db_con->prepare('SELECT e.Client_id, e.FullName, u.Location, u.Client_id, u.Total, u.Status FROM `client` AS e INNER JOIN `requestequipment` AS u ON e.Client_id = u.Client_id AND u.Status="Approved" AND e.Client_id = :uClient_id');
    $stmt->execute(array(":uClient_id"=>$_SESSION['Client_id']));
    $stmt->execute();
