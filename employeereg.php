<?php
	...
	if(isset($_POST['staff']))
	{
		...
		$stmt_check = $db_con->prepare("SELECT Phone FROM employee WHERE Phone=:uphone");
	...
	(!isset($errMSG))
		{
			$stmt = $db_con->prepare('INSERT INTO employee (emp_id,FullName,DateEmployed,Image,Email
			,Address,Phone,Username,Password) VALUES
			(:uemp_id, :uname, :uempyear, :upic,  :uemail, :uaddress, :uphone ,:uusername,:upassword)');
			$stmt->bindParam(':uemp_id',$emp_id);
			$stmt->bindParam(':uname',$name);
			$stmt->bindParam(':uempyear',$empyear);
...
<body>
        ...
        <input type="text" class="form-control" name="emp_id" aria-describedby="emailHelp"  value="<?php $rand = rand(100,999);
echo 'Emp/Staff/'.''.$rand;?>" readonly="disable" required>
		</div>
	    <div class="form-group">
        <input type="text" class="form-control" name="name" aria-describedby="emailHelp" placeholder="Enter FullName" required>
		</div>
	   <div class="form-group">
       <input type="Date" class="form-control" name="empyear" aria-describedby="emailHelp" placeholder="Enter Date Employed" required>
      </div>
	  <div class="form-group">
    ...
    <input type="submit" class="btn btn-success" name="staff" value="Submit" >
</form>
        </div>
        </div>
        </div>
